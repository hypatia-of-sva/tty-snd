...
static uint64_t reverse_all_bits(uint64_t v){
...
static uint64_t reverse_bits(uint64_t num, uint64_t len) {
    assert(len <= 64);
...
static void fft_power_of_two_inplace(double* data, double* dest, size_t len, bool reverse) {
    ...
    for(int l = 0; l < N-1; l++) {
        for(int combined_i = 0; combined_i < len/4; combined_i++) {
			...
			double c = cos(2*M_PI*i_value/(1<<(l+1))),
					s = (reverse ? -1.0 : 1.0)*sin(2*M_PI*i_value/(1<<(l+1)));
			double even_real = data[even_index], even_imag = data[even_index+1];
			double odd_real  = data[odd_index],  odd_imag  = data[odd_index+1];
    ...
    for (int i = 0; i < len/2; i++) {
        ...
        dest[2*i] = data[2*real_index] / (reverse ? (double)(1<<(N-1)) : 1.0f);
        dest[2*i+1] = data[2*real_index+1] / (reverse ? (double)(1<<(N-1)) : 1.0f);
    }
}
